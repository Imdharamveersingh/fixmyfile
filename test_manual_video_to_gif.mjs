/**
 * Phase 6.3 — Video to GIF: Real Chrome CDP Verification
 * Tests Desktop (1440x900) and Mobile (375x667).
 * Run: node test_manual_video_to_gif.mjs
 */
...
async function runViewportTest(label, width, height, debugPort) {
  ...
  try {
    ...
    if (title.includes('Video to GIF')) pass(`Page title: "${title}"`);
    ...
    if (h1.includes('Video to GIF')) pass(`H1 heading: "${h1}"`);
    else fail(`Unexpected H1: "${h1}"`);

    // 2. Dropzone exists
    const dropzoneRes = await send('Runtime.evaluate', { expression: '!!document.getElementById("video-to-gif-dropzone")' });
    ...
    await send('Runtime.evaluate', {
      ...
      expression: `
        (async () => {
          ...
          const file = new File([bytes], 'reaction_clip.mp4', { type: 'video/mp4' });
    ...
    const metaCheck = await send('Runtime.evaluate', {
      expression: `
        JSON.stringify({
          ...
          hasFpsSelect: !!document.getElementById('fps-select'),
          hasResolutionSelect: !!document.getElementById('resolution-select'),
          hasConvertBtn: !!document.getElementById('btn-convert-gif')
        })
      `
    });
    ...
    if (metaParsed.filename === 'reaction_clip.mp4') pass('Uploaded filename rendered correctly');
    else fail(`Expected reaction_clip.mp4, got "${metaParsed.filename}"`);
    if (metaParsed.hasFpsSelect && metaParsed.hasResolutionSelect) pass('FPS & resolution dropdown controls active');
    else fail('FPS or resolution controls missing');
    if (metaParsed.hasConvertBtn) pass('Generate Animated GIF button enabled');
    else fail('Generate button missing');

    // 5. Trigger Real Video to GIF Conversion
    console.log(`    Triggering GIF generation in ${label}...`);
    await send('Runtime.evaluate', {
      expression: `document.getElementById('btn-convert-gif').click()`
    });
    ...
    while (Date.now() - startWait < 30000) {
      const stateCheck = await send('Runtime.evaluate', {
        expression: `
          JSON.stringify({
            hasResult: !!document.getElementById('video-to-gif-result'),
            hasPreview: !!document.getElementById('gif-preview'),
            hasDownload: !!document.getElementById('btn-download-gif'),
      ...
      if (st.hasError) {
        fail('Encountered alert-error during GIF generation');
        break;
      }
      await new Promise((r) => setTimeout(r, 500));
    }

    if (conversionDone) pass('GIF conversion completed successfully and result card appeared');
    else fail('Conversion timed out or failed to show result card');

    // 6. Preview, download link, and filename validation
    const downloadCheck = await send('Runtime.evaluate', {
      expression: `
        JSON.stringify({
          href: document.getElementById('btn-download-gif')?.getAttribute('href') || '',
          download: document.getElementById('btn-download-gif')?.getAttribute('download') || '',
          previewSrc: document.getElementById('gif-preview')?.getAttribute('src') || '',
          statsText: document.querySelector('.stats-comparison')?.innerText || ''
        })
      `
    });
    const dlParsed = JSON.parse(downloadCheck?.result?.value);
    if (dlParsed.href.startsWith('blob:')) pass(`GIF Blob download URL generated: ${dlParsed.href.slice(0, 30)}...`);
    else fail(`Expected blob: download URL, got "${dlParsed.href}"`);
    if (dlParsed.download === 'reaction_clip.gif') pass('Download filename correctly sanitized to reaction_clip.gif');
    else fail(`Unexpected download filename: "${dlParsed.download}"`);
    if (dlParsed.previewSrc.startsWith('blob:')) pass('GIF preview image element rendered with valid source URL');
    else fail('GIF preview missing valid blob source');
    if (dlParsed.statsText.includes('GIF Size') && dlParsed.statsText.includes('Frames')) {
      pass('GIF statistics (size, frame count) displayed');
    } else fail('GIF statistics incomplete');
    ...
    const resetCheck = await send('Runtime.evaluate', {
      expression: `
        JSON.stringify({
          dropzoneVisible: !!document.getElementById('video-to-gif-dropzone'),
          resultGone: !document.getElementById('video-to-gif-result')
        })
      `
    });
    ...
    await send('Runtime.evaluate', {
      ...
      expression: `
        (async () => {
          ...
          const bytes = new Uint8Array(bin.length);
          for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
          const file = new File([bytes], 'second_run.mp4', { type: 'video/mp4' });
    ...
    await send('Runtime.evaluate', { expression: `document.getElementById('btn-convert-gif').click()` });

    let secondDone = false;
    const startWait2 = Date.now();
    while (Date.now() - startWait2 < 30000) {
      const st2 = await send('Runtime.evaluate', {
        expression: `!!document.getElementById('video-to-gif-result')`
      });
      if (st2?.result?.value) {
        secondDone = true;
        break;
    ...
    // Reset after second conversion
    await send('Runtime.evaluate', { expression: `document.getElementById('btn-convert-another').click()` });
...
async function runAllManualTests() {
  console.log('\n=== Phase 6.3 — Video to GIF: Real Chrome CDP Tests ===\n');
  await runViewportTest('Desktop Viewport', 1440, 900, 9480);
  await runViewportTest('Mobile Viewport', 375, 667, 9481);
